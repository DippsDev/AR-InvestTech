...
def main() -> None:
    ...
    p.add_argument("--symbol", default=None,
                   help="MT5 symbol name (default: auto-detect US30, same as the live bot)")
    ...
    symbol = args.symbol
    if not symbol:
        from src.data_collector import find_us30_symbol
        symbol = find_us30_symbol()
        if not symbol:
            print("Could not auto-detect a US30 symbol on this broker — pass --symbol explicitly")
            sys.exit(1)
        print(f"Auto-detected symbol: {symbol}")

    out_path = args.out or f"data/{symbol.lower()}_m5_{args.years}y.csv"

    df = fetch(symbol, args.years)
