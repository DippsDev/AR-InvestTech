...
def quoted(monkeypatch):
    """Quote a fixed spread, and let a test drop the tick entirely."""
    def _quote(spread: float | None):
        if spread is None:
            tick = None
        else:
            bid = 1.15500
            tick = SimpleNamespace(bid=bid, ask=bid + spread)
        monkeypatch.setattr(
            "silver_bullet.live_adapter.mt5_cache.symbol_info_tick",
            lambda symbol: tick,
        )
    return _quote
