...
# Ensure imports resolve regardless of invocation cwd (double-click, autostart).
# Not needed when frozen — PyInstaller's bootloader handles imports itself,
# and __file__ doesn't point at a stable install path in that case anyway.
if not getattr(sys, "frozen", False):
    sys.path.insert(0, str(Path(__file__).parent))

from src import paths  # noqa: E402  (after path fix)

if paths.is_frozen():
    # No sibling pythonw.exe to chdir "next to" — sys.executable IS the app.
    # Writable data lives in the per-user app-data dir, not next to the exe
    # (which is normally installed under Program Files).
    os.chdir(paths.app_data_dir())
else:
...
if paths.is_frozen():
    # The built exe is the whole interpreter + app — launch it directly.
    _AUTOSTART_CMD = f'"{sys.executable}"'
else:
    ...
    _AUTOSTART_CMD = f'"{_PYTHONW}" "{_SCRIPT}"'
